...
export default function Home() {
  ...
  const quickAccess = [
    { title: "我的课程", desc: "继续您的学习旅程", icon: BookOpen, color: "text-blue-600", bg: "bg-blue-50", href: "/login" },
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col justify-center px-6 md:px-16 lg:px-24 max-w-5xl">
              ...
              <div className="mt-10 flex items-center gap-4 animate-fade-in-up animation-delay-300">
                <Link to="/login" className="h-14 px-8 inline-flex items-center justify-center rounded-full bg-slate-900 text-white text-base font-medium hover:bg-slate-800 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
      ...
      <div className="w-full px-6 md:px-12 lg:px-24 max-w-[1600px] mx-auto py-24 space-y-32">
        ...
        <section>
          <div className="flex items-end justify-between mb-12">
            ...
            <Link to="/login" className="hidden md:flex items-center gap-2 text-blue-600 font-medium hover:text-blue-700 transition-colors group">
              探索全部课程 <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Link>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 lg:gap-8">
            {courses.map((course, i) => (
              <Link to="/login" key={i} className="group block bg-white rounded-3xl overflow-hidden shadow-[0_2px_20px_rgb(0,0,0,0.02)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] transition-all duration-500 hover:-translate-y-2 border border-slate-100">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
          ...
          <section className="lg:col-span-1 bg-white rounded-3xl p-8 shadow-[0_2px_20px_rgb(0,0,0,0.02)] border border-slate-100">
            <div className="flex items-center justify-between mb-8">
              ...
              <Link to="/login" className="text-sm text-blue-600 font-medium hover:text-blue-700 transition-colors">
                全部
              </Link>
            </div>
            <div className="space-y-6">
              {announcements.map((announcement, i) => (
                <Link to="/login" key={i} className="group flex gap-4 items-start">
